require "csv"
task :load_w10_sa => :environment do
  hash = {}
  CSV.foreach(File.expand_path('../w10_sa.csv', __FILE__)) do |row|
    pid = row[0]
    sa_grade = row[1].to_i
    hash[pid] = sa_grade
  end
    ...
    if team.present?
      ...
      pids.each do |s_pid|
        if hash[s_pid].present?
          ...
          #Make sure assignment 10 has the last grading field as out of box
